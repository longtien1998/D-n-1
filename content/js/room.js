...
function showimg(id) {
  const im = document.getElementById(0);
  const im1 = document.getElementById(id);
  const previews4 = document.querySelectorAll(".room4-preview4 img");
  const im2 = im1.getAttribute('src');
  im.setAttribute('src', im2);
  previews4.forEach(preview4 => preview4.classList.remove("room4-active4"));
  im1.classList.add("room4-active4");
}